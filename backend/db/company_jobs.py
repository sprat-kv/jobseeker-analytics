...
class CompanyJobs(SQLModel, table=True):
    ...
    company_job_title_id: int | None = Field(default=None, foreign_key="job_titles.job_title_id", nullable=True)
    company_job_description: str | None = Field(default=None, nullable=True)
    company_job_posted_at: datetime = Field(default_factory=datetime.utcnow, nullable=False) 
