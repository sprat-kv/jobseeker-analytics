...
def parse_simple(term: str, field: str, exclude: bool = False) -> str:
    """
...
def parse_wildcard(term: str, field: str, exclude: bool = False) -> str:
    """
...
def parse_base_filter_config(filter_path: str) -> str:
    ...
    for block in data:
        ...
        if block["how"] == "include":
            simple_filters += [
                parse_simple(x, block["field"], exclude=False)
                for x in block["terms"]
                if "*" not in x
            ]
            wildcard_any_filters += [
                parse_wildcard(x, block["field"], exclude=False)
                for x in block["terms"]
                if "*" in x
            ]
        if block["how"] == "exclude":
            simple_filters += [
                parse_simple(x, block["field"], exclude=True) for x in block["terms"]
            ]
    ...
    for block in data:
        ...
        for sub_block in block:
            ...
            if include_terms is not None:
                simple_filters += [
                    parse_simple(x, sub_block["field"], exclude=False)
                    for x in sub_block["include_terms"]
                ]
            if exclude_terms is not None:
                simple_filters += [
                    parse_simple(x, sub_block["field"], exclude=True)
                    for x in sub_block["exclude_terms"]
                ]
