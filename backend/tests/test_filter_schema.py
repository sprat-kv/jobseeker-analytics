...
def get_base_filter_config_paths() -> List[Path]:
    return [SAMPLE_FILTER_PATH] + [
        x for x in FITLER_CONFIG_DIR.iterdir() if "override" not in str(x)
    ]
    ...
    for block in filter_config:
        if block["field"] == "subject":
            ...
            if block["logic"] == "any" and block["how"] == "include":
                ...
                if not ret_val:
                    ret_val = any(
                        [
                            re.findall(
                                x.replace(" * ", ".*").lower(), subject_text.lower()
                            )
                            for x in block["terms"]
                            if "*" in x
                        ]
            ...
            if ret_val:
                if block["logic"] == "all" and block["how"] == "exclude":
                    ret_val = all(
                        [x.lower() not in subject_text.lower() for x in block["terms"]]
                    )
        ...
        if block["field"] == "from":
            ...
            if block["logic"] == "any" and block["how"] == "include":
                ...
                if not ret_val:
                    ret_val = any(
                        [
                            re.findall(
                                x.replace(" * ", ".*").lower(), from_text.lower()
                            )
                            for x in block["terms"]
                            if "*" in x
                        ]
            ...
            if ret_val:
                if block["logic"] == "all" and block["how"] == "exclude":
                    ret_val = all(
                        [x.lower() not in from_text.lower() for x in block["terms"]]
                    )
...
def test_apply_email_filter_desired_fail(test_constant, filter_config):
    ...
    for subject_text, from_text in test_constant:
        ...
        if result:
            print(
                f"(subject: {subject_text}, from: {from_text} passed, which is undesired"
            )
